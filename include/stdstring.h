...
//
//	REVISION HISTORY
//    2014-FEB-18 - fix compare/Compare
//    2013-DEC-24 - Add Tokenize(),Append(),AppendChar()
//                  - by HyungKi Jeong (clonextop@gmail.com)
//
...
public:
	...
	#define SSRES(pctstr)  LOWORD(reinterpret_cast<DWORD_PTR>(pctstr))
	...
	bool TryLoad(const void* pT)
	{
		...
		{
			DWORD nId = LOWORD(reinterpret_cast<DWORD_PTR>(pT));
			if ( !LoadString(nId) )
			{
...
//		}
	...
	// Tokenize
	CStdStr<CT> Tokenize(PCMYSTR szTokens, int& iStart) const
	{
		ASSERT(iStart >= 0);
		CStdStr<CT>	str;
		PCMYSTR	szPlace	= GetString() + iStart;
		PCMYSTR	szEnd	= GetString() + _tcslen(this->c_str());
		if(szPlace < szEnd){
			str	= szPlace;
			int nIncluding	= str.SpanIncluding(szTokens).GetLength();
			if((szPlace+nIncluding) < szEnd){
				szPlace	+= nIncluding;
				str		= szPlace;
				int nExcluding	= str.SpanExcluding(szTokens).GetLength();
	...
	{
		MYSIZE nIdx	= this->rfind(szFind, pos);
		return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
	}
